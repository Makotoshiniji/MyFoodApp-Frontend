...
export default function AdminNewFoodScreen({ route, navigation }: any) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (image) {
        // 🟡 LOG 1: ดูข้อมูลทั้งหมดใน Object image
        console.log('📸 IMAGE OBJECT DATA:', JSON.stringify(image, null, 2));

        // 🟡 LOG 2: ดู Path เต็มๆ ที่ React Native ได้มา
        console.log('📍 FULL PATH (URI):', image.uri);
      ...
      // ... (โค้ดส่วนเรียก API เหมือนเดิม) ...
      await api.post(`/menuitems`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });

      // ... (ส่วน Alert Success เหมือนเดิม) ...
      Alert.alert('สำเร็จ', 'เพิ่มเมนูใหม่เรียบร้อย', [
        {
          text: 'OK',
          onPress: () => {
