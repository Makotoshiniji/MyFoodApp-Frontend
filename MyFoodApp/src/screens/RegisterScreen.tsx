...
export default function RegisterScreen({ onRegistered, onGoSignIn }: Props) {
  ...
  const submit = async () => {
    ...
    if (!username.trim() || !email.trim() || !password || !confirmPassword) {
      Alert.alert('สมัครไม่สำเร็จ', 'กรุณากรอกข้อมูลให้ครบทุกช่อง');
      return;
    }

    // 2. เช็คเงื่อนไขรหัสผ่าน (พิมพ์เล็ก, พิมพ์ใหญ่, ตัวเลข, ความยาว 8 ตัวขึ้นไป)
...
const styles = StyleSheet.create({
  ...
  passwordHintText: {
    fontSize: 11,
    color: COLOR.gray,
    fontFamily: 'Montserrat-Light', // หรือฟอนต์ที่คุณใช้
  },
});
