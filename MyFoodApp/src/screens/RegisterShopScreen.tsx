...
export default function RegisterShopScreen({ route, navigation }: any) {
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      // ✅ 1. รับค่า Response กลับมาจาก API เพื่อเอา shopId ใหม่
      ...
      // ดึง ID ร้านที่เพิ่งสร้างสำเร็จ (ตรวจสอบชื่อฟิลด์จาก Backend ของคุณ เช่น res.id หรือ res.data.id)
      const newShopId = res.id || res.data?.id;

      Alert.alert('ยินดีด้วย!', 'ร้านค้าของคุณเปิดใช้งานแล้ว', [
        {
          text: 'ไปที่แดชบอร์ด',
          onPress: () =>
            // ✅ 2. ส่งทั้ง userId และ shopId ใหม่ไปยังหน้า Admin
            navigation.replace('MainAdminShop', {
              userId,
              shopId: newShopId,
            }),
        },
